...
public:
	...
	AudioGroupFMOD( AudioMgrFMOD &audioMgr, const utf8string &grpName ) 
	...
	virtual void SetPause( const bool state, const unsigned int channelHandle = UINT_MAX );
	virtual bool Initialise( const bool asStreams );
	virtual bool Load( const utf8string &folder );
	...
	virtual bool Play( const utf8string &name, const bool loop, const bool paused, unsigned int *pChannelHandle = NULL,
						const float vol = 1.f );
	virtual bool Play( const unsigned int soundHandle, const bool loop, const bool paused, unsigned int *pChannelHandle = NULL,
						const float vol = 1.f );
	virtual bool Exists( const utf8string &name, int *pSoundIndex = NULL );
	virtual bool IsPlaying( const unsigned int channelHandle );
	virtual const utf8string &GetName( const unsigned int channelHandle );
...
private:
	...
	{
		FMOD::Sound *_pSound;
		utf8string _name;	//lets us link a sound to its friendly name
	};
	...
	int GetSoundData( const std::string &name, SoundData ** pSD = nullptr );
	///get channel data
