// by Mark Featherstone (C) 2020 All Rights Reserved.
#ifndef AUDIOMGR_H
#define AUDIOMGR_H
...
public:
	...
	explicit IAudioGroup( const utf8string &grpName ) 
	...
	virtual bool Load( const utf8string &folder ) =0;
	...
	///pause one channel
	virtual void SetPause( const bool state, const unsigned int channelHandle = UINT_MAX) =0;
	...
	virtual bool Play( const utf8string &name, const bool loop, const bool paused, unsigned int * pChannelHandle = nullptr, 
	...
	virtual bool Play( const unsigned int soundHandle, const bool loop, const bool paused, unsigned int * pChannelHandle = nullptr, 
						const float vol = 1.f ) =0;
	///does the named sound exist in the group
	virtual bool Exists( const utf8string &name, int *pSoundIndex = nullptr ) =0;
	...
	virtual const utf8string &GetName( const unsigned int channelHandle ) =0;
	...
	const IAudioGroup &operator=(const IAudioGroup &) = delete;  //prevent an audio group being assigned
protected:
	utf8string m_grpName;			///< friendly name for group
...
{
public:
	IAudioMgr() : m_pSfxMgr(nullptr), m_pSongMgr(nullptr) { } 
	virtual ~IAudioMgr() {
		Shutdown();
	}
